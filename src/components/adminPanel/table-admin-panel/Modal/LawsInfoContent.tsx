...
const LawsInfoContent = ({formik, openInNewTab}: Props) => {
    return (
        <Paper elevation={3} className={'flex flex-col gap-5 p-6 w-full'}>
            ...
            <div>
                ...
                {YesNoToBool(formik.values.hasVisaBefore) && <div className={'my-4'}>
                    ...
                    {formik.values.hasVisaBeforeData.map((el:string, i:number) => (
            ...
            <div>
                ...
                {YesNoToBool(formik.values.hasLivedInNonResidenceCountry) && <div>
                    ...
                    <div>
                        {formik.values.hasLivedInNonResidenceCountryData.map((el:{ country: string, date: string, address: string }, index:number) => {
                            return (
                                    ...
                                    <TextField className={'mt-2 w-[200px]'}
                                               ...
                                               onChange={(e) => {
                                                   ...
                                                   const data = formik.values.hasLivedInNonResidenceCountryData.map(
                                                       (el:{ country: string, date: string, address: string }, ind:number) => ind === index ? ({
                                                           ...el,
                                                           [key]: e.target.value
                                                       }) : el);
            ...
            <div className={'flex gap-5 justify-between'}>
                ...
                <div>
                    ...
                    {YesNoToBool(formik.values.hasVisaReject) && <div>
                        ...
                        {formik.values.hasVisaRejectData.map((el:string, i:number) => (
                            <IconButton onClick={() => openInNewTab(el)} key={i}>
                                <PhotoIcon fontSize={'large'}/>
                            </IconButton>
                        ))}
                ...
                <div>
                    ...
                    <>
                        {YesNoToBool(formik.values.hasAnotherPassport) && <div>
                            ...
                            {formik.values.hasAnotherPassportData.map((el:string, i:number) => (
                                <IconButton onClick={() => openInNewTab(el)} key={i}>
                                    <PhotoIcon fontSize={'large'}/>
                                </IconButton>
                            ))}
...
export default LawsInfoContent;